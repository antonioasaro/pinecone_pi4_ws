  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  if (last_command_msg->twist.linear.x != 0) { RCLCPP_INFO(logger, "Hey antonio, got an update %f", last_command_msg->twist.linear.x); }
  const auto age_of_last_command = time - last_command_msg->header.stamp;
  // Brake if cmd_vel has timeout, override the stored command
  if (age_of_last_command > cmd_vel_timeout_)
  {
    if (last_command_msg->twist.linear.x != 0) { RCLCPP_INFO(logger, "Hey antonio, command is too old"); }
    if (last_command_msg->twist.linear.x != 0) { RCLCPP_INFO(logger, "Hey antonio, command age %ld %ld", age_of_last_command, cmd_vel_timeout_.count()); }
  ...
  {
  if (last_command_msg->twist.linear.x != 0) { RCLCPP_INFO(logger, "Hey antonio, open_loop"); }
    odometry_.updateOpenLoop(linear_command, angular_command, time);
  }
  else
  {
    if (last_command_msg->twist.linear.x != 0) { RCLCPP_INFO(logger, "Hey antonio, close_loop"); }
  ...
  if (last_command_msg->twist.linear.x != 0) { RCLCPP_INFO(logger, "Hey antonio, should_publish==false here"); }
  try
  {
  ...
  if (should_publish)
  {
    if (last_command_msg->twist.linear.x != 0) { RCLCPP_INFO(logger, "Hey antonio, should_publish==true"); }
    previous_publish_timestamp_ += publish_period_;
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  {
    params_ = param_listener_->get_params();
    RCLCPP_INFO(logger, "Hey antonio, Parameters were updated");
  }
